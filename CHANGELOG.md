...
- `burn measure` and `@relayburn/sdk.measureSession()` turn one explicit Claude Code, Codex, or OpenCode session artifact into a versioned per-model token/cost document without discovery or a ledger.
