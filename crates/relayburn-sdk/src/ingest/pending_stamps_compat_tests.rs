...
fn rust_parses_ts_written_stamp() {
    ...
    let parsed =
        crate::ingest::pending_stamps::parse_pending_stamp(&raw).expect("ts fixture parses");
    assert_eq!(parsed.v, 1);
    assert_eq!(parsed.harness, crate::ingest::PendingStampHarness::Codex);
...
fn rust_reserialization_is_byte_identical_to_ts_fixture() {
    let raw = std::fs::read_to_string(fixture_path("ts_codex_stamp.json")).unwrap();
    let parsed = crate::ingest::pending_stamps::parse_pending_stamp(&raw).unwrap();
    let reserialized = crate::ingest::pending_stamps::serialize_stamp(&parsed);
    assert_eq!(
        reserialized, raw,
...
fn rejects_wrong_version() {
    ...
    assert!(
        crate::ingest::pending_stamps::parse_pending_stamp(raw).is_none(),
...
fn rejects_unknown_harness() {
    ...
    assert!(crate::ingest::pending_stamps::parse_pending_stamp(raw).is_none());
}
