...
use crate::reader::{ContentKind, ContentRecord, ContentStoreMode, TurnRecord};
...
mod tests {
    ...
    // mutate it. The shared `TEST_GAP_LOCK` (declared in `crate::ingest`)
    // is used here too — without that, this module's tests would race
    // gap_warning_tests, which mutate the same global tracker. A
    // panicking test poisons the lock; recover the inner value so a
    // peer test still runs.
    fn test_lock() -> MutexGuard<'static, ()> {
        super::super::TEST_GAP_LOCK
            .lock()
            .unwrap_or_else(|e| e.into_inner())
    }
